...
loop: 	add $t2, $t0, $t0 #put 2i in $t2
	...
	slt $t5, $v1, $t1 #set $t5 to 1 if max sum < new_sum
	bne $t5, $zero, mdfy #if max_sum is less, modify results
	j test #done?
mdfy: 	addi $v0, $t0, 1 #new max_sum prefix has length i+1
	addi $v1, $t1, 0 #new_max is the running sum
